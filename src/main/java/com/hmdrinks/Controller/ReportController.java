...
public class ReportController {
    ...
    public ResponseEntity<?> getSuccessfulOrdersReport(
            @RequestParam("year") int year,
            @RequestParam("month") int month) {
        return ResponseEntity.ok(reportService.getSuccessfulOrdersCount(year, month));
    }

    @GetMapping("/order/cancelled")
    public ResponseEntity<?> getCancelledOrdersReport(
            @RequestParam("year") int year,
            @RequestParam("month") int month) {
        return ResponseEntity.ok(reportService.getCancelledOrdersCount(year, month));
    }
