...
public class AdminController {
    ...
    @GetMapping("/list-payment-refund/status")
    public ResponseEntity<?> handleListPaymentRefundByStatus(
            @RequestParam(name = "page") String page,
            @RequestParam(name = "limit") String limit,
            @RequestParam(name = "status") Boolean isRefund) {
        ResponseEntity<?> validationResult = supportFunction.validatePaginationParams(page, limit);

        if (validationResult != null) {
            return validationResult;  // return luôn nếu có lỗi
        }
        return  paymentService.listAllPaymentRefundByStatus(page, limit,isRefund);
    }

    @PutMapping("/activate/refund")
    public ResponseEntity<?> handleRefund(
            @RequestBody @Valid IdReq idReq,HttpServletRequest httpRequest) {
        String token = httpRequest.getHeader("Authorization");
        return  paymentService.activateRefund(idReq.getId(),token);
    }
    ...
    public ResponseEntity<?> handleRefundGroup(
            @RequestBody IdReq idReq, HttpServletRequest httpRequest) {
        String token = httpRequest.getHeader("Authorization");
        return  paymentGroupService.activateRefundGroup(idReq.getId(),token);
    }
