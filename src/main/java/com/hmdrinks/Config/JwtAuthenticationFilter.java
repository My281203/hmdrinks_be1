        ...
        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (userDetails != null) {
                ...
                if (jwtService.isTokenValid(jwt, userDetails) && isTokenValid) {
                    String sessionIdFromToken = jwtService.extractClaim(jwt, claims -> claims.get("sessionId", String.class));
                    String userId = jwtService.extractUserId(jwt);
                    Token tokenInDb = tokenRepository.findByUserUserId(Integer.parseInt(userId));

                    if (tokenInDb == null || !sessionIdFromToken.equals(tokenInDb.getSessionId())) {
                        msg = "Session ID không hợp lệ hoặc đã bị thay thế – bạn đã đăng nhập ở nơi khác.";
                        customStatus = HttpServletResponse.SC_UNAUTHORIZED;
                    } else {
