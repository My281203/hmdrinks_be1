...
@Service
public class CartItemGroupLeaderService {
    @Autowired
    ...
    @Transactional
    public ResponseEntity<?> insertCartItemLeader(InsertItemToCartLeader req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null)
        {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("UserLeader Not Found");
        }

        User userMember = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdMember());
        if(userMember == null)
        {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("UserMember Not Found");
        }
        GroupOrders groupOrder1 = groupOrdersRepository.findByGroupOrderIdAndIsDeletedFalse(req.getGroupOrderId());
        if (groupOrder1 == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Group not found");
        }
        LocalTime localTime = LocalTime.now();
        LocalTime groupTime = groupOrder1.getDatePaymentTime(); // ví dụ: 09:20

// Nếu now >= groupTime thì không cho phép

        if(groupOrder1.getTypeTime() == Status_Type_Time_Group.TIME)
        {
        ...
        GroupOrderMember leader = groupOrderMembersRepository.findByGroupOrderGroupOrderIdAndUserUserId(req.getGroupOrderId(), req.getUserIdLeader());
        ...
        if(cartItem1 == null)
        {
            ...
            List<CartItemGroup> cartItemList = cartItemGroupRepository.findByCartGroupCartIdAndIsDisabledFalse(cartItem.getCartGroup().getCartId());
            Double Price = 0.0;
            Integer Quantity=0;
...
// Cập nhật lại quantity và totalPrice
            ...
            cartItem1.setDateCreated(LocalDateTime.now());
...
// Cập nhật tổng số lượng và giá tiền của giỏ hàng
    ...
    @Transactional
    public ResponseEntity<?> increaseCartItemGroupQuantityLeader(IncreaseDecreaseItemQuantityLeaderReq req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null)
        {
...
// Nếu now >= groupTime thì không cho phép
    ...
    @Transactional
    public ResponseEntity<?> updateCartItemQuantity(IncreaseDecreaseItemQuantityLeaderReq req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("UserLeader Not Found");
...
// Nếu now >= groupTime thì không cho phép
        ...
        ProductVariants productVariants = productVariantsRepository.findByVarId(cartItem.getProductVariants().getVarId());
        int presentQuantity = req.getQuantity();
        double presentTotalPrice = productVariants.getPrice() * presentQuantity;

        if(presentQuantity > productVariants.getStock()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Quantity is greater than stock");
        }
        ...
        GroupOrders groupOrdersCheck = cart.getGroupOrderMember().getGroupOrder();
        if(groupOrdersCheck.getStatus() == StatusGroupOrder.COMPLETED ||
                groupOrdersCheck.getStatus() == StatusGroupOrder.CHECKOUT ||
                groupOrdersCheck.getStatus() == StatusGroupOrder.CANCELED) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Group Order Completed or Canceled");
        }

        // ✅ Cập nhật cart item
        cartItem.setQuantity(presentQuantity);
        cartItem.setTotalPrice(presentTotalPrice);
        cartItem.setDateUpdated(LocalDateTime.now());
        cartItemGroupRepository.save(cartItem);

        // ✅ Cập nhật lại cart group
        ...
        // ✅ Cập nhật lại group order
        GroupOrders groupOrders = groupOrdersRepository.findByGroupOrderIdAndIsDeletedFalse(groupOrdersCheck.getGroupOrderId());
        double priceGroup = 0.0;
        int quantityGroup = 0;
        for (GroupOrderMember member : groupOrders.getGroupOrderMembers()) {
            if (member.getAmount() != null) priceGroup += member.getAmount();
            if (member.getQuantity() != null) quantityGroup += member.getQuantity();
        }

        groupOrders.setTotalQuantity(quantityGroup);
        groupOrders.setTotalPrice(priceGroup);
        groupOrders.setStatus(StatusGroupOrder.SHOPPING);
        groupOrders.setDateUpdated(LocalDateTime.now());
        groupOrdersRepository.save(groupOrders);

        // 🔔 Gửi thông báo cho các thành viên khác trong nhóm (trừ leader)
        try {
            String message = userLeader.getFullName() + " đã cập nhật số lượng " +cartItem.getProductVariants().getProduct().getProName()+" bạn chọn thành "+ req.getQuantity();
            groupOrders.getGroupOrderMembers().stream()
                    .filter(m -> !m.getIsDeleted() && !m.getUser().getUserId().equals(req.getUserIdLeader()))
                    .forEach(m -> {
                        try {
                            Integer receiverId = m.getUser().getUserId();
                            notificationService.sendProductLeaderUpdateNotification(receiverId, groupOrders.getGroupOrderId(), message);
            ...
            System.err.println("❌ Không thể gửi thông báo cập nhật cart item: " + e.getMessage());
        }
    ...
    @Transactional
    public ResponseEntity<?> changeSizeCartItemQuantity(ChangeSizeItemLeaderReq req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null)
        {
...
// Nếu now >= groupTime thì không cho phép
        ...
        try {
            String message = userLeader.getFullName() + " đã cập nhật "+cartItem.getProductVariants().getProduct().getProName()+" bạn chọn thành size "+ req.getSize();
            groupOrders.getGroupOrderMembers().stream()
                    .filter(m -> !m.getIsDeleted() && !m.getUser().getUserId().equals(req.getUserIdLeader()))
                    .forEach(m -> {
                        try {
    ...
    @Transactional
    public ResponseEntity<?> decreaseCartItemQuantity(IncreaseDecreaseItemQuantityLeaderReq req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null)
        {
...
// Nếu now >= groupTime thì không cho phép
    ...
    @Transactional
    public ResponseEntity<?> deleteOneItem(DeleteOneCartItemLeaderReq req)
    {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if(userLeader == null)
        {
...
// Nếu now >= groupTime thì không cho phép
    ...
    @Transactional
    public ResponseEntity<?> deleteAllCartItem(DeleteAllCartItemLeaderReq req) {
        User userLeader = userRepository.findByUserIdAndIsDeletedFalse(req.getUserIdLeader());
        if (userLeader == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("UserLeader Not Found");
        }
...
// Nếu now >= groupTime thì không cho phép
        ...
        GroupOrderMember leader = groupOrderMembersRepository.findByGroupOrderGroupOrderIdAndUserUserId(
                req.getGroupOrderId(), req.getUserIdLeader());
        if (leader == null || !Boolean.TRUE.equals(leader.getIsLeader())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Leader not found");
        }
        ...
        GroupOrders groupOrdersCheck = cartRestore.getGroupOrderMember().getGroupOrder();
        if (groupOrdersCheck.getStatus() == StatusGroupOrder.COMPLETED ||
                groupOrdersCheck.getStatus() == StatusGroupOrder.CHECKOUT ||
                groupOrdersCheck.getStatus() == StatusGroupOrder.CANCELED) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Group Order Completed or Canceled");
        ...
        GroupOrders groupOrders1 = groupOrdersRepository.findByGroupOrderIdAndIsDeletedFalse(groupOrdersCheck.getGroupOrderId());
        double priceGroup = 0.0;
        int quantityGroup = 0;
        ...
        // 🔔 Gửi thông báo đến tất cả các thành viên khác (trừ leader)
        try {
            String message = "A member's cart has been cleared by the leader";
            List<GroupOrderMember> memberList = groupOrders1.getGroupOrderMembers();

            for (GroupOrderMember member : memberList) {
                Integer receiverId = member.getUser().getUserId();
                if (!receiverId.equals(req.getUserIdLeader())) {
                    notificationService.sendProductUpdateNotification(receiverId, req.getGroupOrderId(), message);
                    System.out.println("📨 Gửi thông báo tới userId=" + receiverId + " với message=" + message);
                }
            }
        } catch (Exception e) {
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(new DeleteCartItemResponse("Delete all item success"));
    }
