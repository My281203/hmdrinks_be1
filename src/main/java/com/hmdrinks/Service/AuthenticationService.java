    ...
    public ResponseEntity<?> authenticate(LoginBasicReq request) {
        try {
            ...
            if (token == null) {
                token = new Token();
                token.setUser(user);
            } else if (token.getSessionId() != null) {
                String message = "Tài khoản của bạn đã đăng nhập ở nơi khác";
                Integer userId = token.getUser().getUserId();

                try {
                    notificationService.sendNotification(userId, 999, message);
                } catch (Exception ex) {
            ...
            var jwtToken = jwtService.generateToken(claims, myUserDetails, String.valueOf(user.getUserId()), user.getRole().toString());
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR_500).body("Internal error occurred while logging in. " + e.getMessage());
        }
    }
