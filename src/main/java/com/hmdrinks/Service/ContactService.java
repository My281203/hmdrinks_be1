...
public class ContactService {
    ...
    public ResponseEntity<?> listAllContact(String pageFromParam, String limitFromParam) {
        int page = Integer.parseInt(pageFromParam);
        int limit = Math.min(Integer.parseInt(limitFromParam), 100);
        Pageable pageable = PageRequest.of(page - 1, limit);

        Page<ContactProjection> contactsPage = contactRepository.findAllProjected(pageable);
        long totalCount = contactRepository.countAllContacts();

        List<CRUDContactResponse> responses = contactsPage.getContent().stream()
                .map(c -> new CRUDContactResponse(
                        c.getContactId(),
                        c.getDescription(),
                        c.getStatus(),
                        c.getIsDeleted(),
        ...
        return ResponseEntity.ok(new ListAllContactResponse(
                page,
    ...
    public ResponseEntity<?> listAllContactWaiting(String pageFromParam, String limitFromParam)
    {
        int page = Integer.parseInt(pageFromParam);
    ...
    public ResponseEntity<?> listAllContactComplete(String pageFromParam, String limitFromParam)
    {
        int page = Integer.parseInt(pageFromParam);
        int limit = Integer.parseInt(limitFromParam);
        if (limit >= 100) limit = 100;
