...
class PostControllerTest {
    ...
    void createPost_Success() throws Exception {
        ...
        CreateNewPostReq req = new CreateNewPostReq("string","string","string","string", Type_Post.EVENT,3, Language.VN);
    ...
    void getOnePost_NotFound() throws Exception {
        int postId = 1;
        when(postService.getPostById(anyInt(),ArgumentMatchers.any(Language.class)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.NOT_FOUND).body("Not found post"));

        mockMvc.perform(get("/api/post/view/{id}?&language=VN",1))
    ...
    void getOnePost_Success() throws Exception {
        ...
        when(postService.getPostById(anyInt(),ArgumentMatchers.any(Language.class)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.OK).body(response));
        mockMvc.perform(get("/api/post/view/{id}?&language=VN", 1)) // Sử dụng HTTP GET thay vì POST
    ...
    void updatePost_Success() throws Exception {
        ResponseEntity<?> mockAuthResponse = ResponseEntity.ok().build();
        when(supportFunction.checkUserAuthorization(any(HttpServletRequest.class), anyInt())).thenReturn((ResponseEntity) mockAuthResponse);
        CRUDPostReq req = new CRUDPostReq(1,"string","string","string","string", Type_Post.EVENT,3,Language.VN);
        CRUDPostResponse response = new CRUDPostResponse(
                1,
                Type_Post.EVENT,
                "string",
    ...
    void listPostByTye_Success() throws Exception {
        ...
        when(postService.getAllPostByType(anyString(), anyString(),any(Type_Post.class),ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.ok(listAllPostResponse));

        mockMvc.perform(MockMvcRequestBuilders.get("/api/post/view/type/all")
                .param("page", "1")
                .param("limit", "10")
                .param("type", "EVENT")
                        .param("language", Language.VN.toString()))

                .andExpect(status().isOk())
                .andExpect(jsonPath("$.body.listPosts.length()").value(2))
                .andExpect(jsonPath("$.body.listPosts[0].postId").value(1))
                .andDo(print());
    }
    ...
    void listPost_Success() throws Exception {
        ...
        when(postService.getAllPost(anyString(), anyString(),ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity)ResponseEntity.ok(listAllPostResponse));
    ...
    void listPostByUserId_Success() throws Exception {
        ...
        when(postService.listAllPostByUserId(anyInt(),ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.OK).body(listAllPostResponse));

        mockMvc.perform(MockMvcRequestBuilders.get("/api/post/view/author/{userId}?language=VN",3)
                       )
                .andExpect(status().isOk())
    ...
    void listPostByUserId_NotFound() throws Exception {
        when(postService.listAllPostByUserId(anyInt(), ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.NOT_FOUND).body("Not found user"));
