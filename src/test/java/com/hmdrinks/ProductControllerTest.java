...
class ProductControllerTest {
    ...
    void createProduct_Success() throws Exception {
        CreateProductReq createProductReq = new CreateProductReq(1,"Product1","","",Language.VN);
    ...
    void getOneProduct_NotFound() throws Exception {
        ...
        when(productService.getOneProduct(anyInt(),ArgumentMatchers.any(Language.class)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.NOT_FOUND).body("product not exists"));

        mockMvc.perform(MockMvcRequestBuilders.get(endPointPath + "/view/1?language=VN")) // Sử dụng HTTP GET thay vì POST
                .andExpect(status().isNotFound())
    ...
    void listProduct_Success() throws Exception {
        ...
        when(productService.listProduct(anyString(), anyString(), ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.ok().body(listProductResponse));

        mockMvc.perform(MockMvcRequestBuilders.get(endPointPath + "/list-product?page=1&limit=1&language=VN"))
                .andExpect(status().isOk())
    ...
    void TotalSearchProduct_Success() throws Exception {
        ...
        when(productService.totalSearchProduct(anyString(),anyString(),anyString(), ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.OK).body(listCategoryResponse));
