...
class UserControllerTest {
    ...
    void updateUser_Successfully() throws Exception {
        ...
        UserInfoUpdateReq req = new UserInfoUpdateReq(
                ...
                "MALE",
                date,
                "99 đường số 7, Linh Trung, Thành phố Thủ Đức, Thành phố Hồ Chí Minh"
        );
    ...
    @Test
    void enableUser_Successfully() throws Exception {
        IdReq req = new IdReq();
        req.setId(1);
        String requestBody = objectMapper.writeValueAsString(req);
        CRUDAccountUserResponse mockResponse = new CRUDAccountUserResponse(
                1,
                "username",
                "Full Name",
                "",
        ...
        when(userService.enableAccount(eq(1)))
                .thenReturn((ResponseEntity) ResponseEntity.ok(mockResponse));  // ép kiểu rõ ràng

        mockMvc.perform(put(endPointPath + "/enable")
    ...
    @Test
    void disableUser_Successfully() throws Exception {
        IdReq req = new IdReq();
        req.setId(1);
        String requestBody = objectMapper.writeValueAsString(req);
        CRUDAccountUserResponse mockResponse = new CRUDAccountUserResponse(
        ...
        when(userService.disableAccount(eq(1)))
                .thenReturn((ResponseEntity) ResponseEntity.ok(mockResponse));

        mockMvc.perform(put(endPointPath + "/disable")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.userId").value(1))
    ...
    void disableUser_UserNotFound() throws Exception {
        ...
        when(userService.disableAccount(eq(999)))
    ...
    void enableUser_UserNotFound() throws Exception {
        ...
        when(userService.enableAccount(eq(999)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.NOT_FOUND).body("User not found"));
    ...
    @Test
    void enableUser_AlreadyEnabled() throws Exception {
        IdReq req = new IdReq();
        req.setId(1);
        String requestBody = objectMapper.writeValueAsString(req);

        when(userService.enableAccount(eq(1)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.BAD_REQUEST).body("User already enable"));

        mockMvc.perform(put(endPointPath + "/enable")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("User already enable"))
                .andDo(print());
    }

    @Test
    void disableUser_AlreadyDisabled() throws Exception {
        IdReq req = new IdReq();
        req.setId(1);
        String requestBody = objectMapper.writeValueAsString(req);

        when(userService.disableAccount(eq(1)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.BAD_REQUEST).body("User already disabled"));

        mockMvc.perform(put(endPointPath + "/disable")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("User already disabled"))
                .andDo(print());
    }
