...
class ProductVarControllerTest {
    ...
    void updateProduct_Success() throws Exception {
        CRUDProductVarReq crudProductVarReq = new CRUDProductVarReq(
                1,
                1,
                Size.S,
                25000.0,
        ...
        when(productVarService.updateProduct(any(CRUDProductVarReq.class)))
                .thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.OK).body(response));
        String requestBody = objectMapper.writeValueAsString(crudProductVarReq);
        mockMvc.perform(put(endPointPath + "/update")
                        .contentType(MediaType.APPLICATION_JSON_VALUE)
                        .content(requestBody))
                .andDo(print())
    ...
    void listProduct_Success() throws Exception {
        ...
        ResponseEntity<?> response =  ResponseEntity.ok(new ListProductVarResponse(
                1,
                2,
                1,
                2,
        ...
        when(productVarService.listProduct(anyString(), anyString())).thenReturn((ResponseEntity)response);

        mockMvc.perform(MockMvcRequestBuilders.get(endPointPath + "/list-product?page=1&limit=1"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.body.productVarResponses.length()").value(2))
                .andExpect(jsonPath("$.body.productVarResponses[0].price").value(25000.0))
                .andExpect(jsonPath("$.body.productVarResponses[1].size").value("L"))
                ;
    }
}
