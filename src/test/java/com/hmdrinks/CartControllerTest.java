...
class CartControllerTest {
    ...
    @Test
    void createCartAI_Success() throws Exception {
        ResponseEntity<?> mockAuthResponse = ResponseEntity.ok().build();
        ...
        CreateNewCart req = new CreateNewCart();
        req.setUserId(1);

        CreateNewCartResponse mockResponse = new CreateNewCartResponse(
                1, 0.0, 0, 1, Status_Cart.CART_AI.name()
        );
        when(cartService.createCartAI(any(CreateNewCart.class)))
                .thenReturn((ResponseEntity) ResponseEntity.ok(mockResponse));

        String requestBody = objectMapper.writeValueAsString(req);

        mockMvc.perform(post(endPointPath +"/createAI")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.userId").value(1))
                .andExpect(jsonPath("$.statusCart").value(Status_Cart.CART_AI.name()))
                .andDo(print());
    }

    @Test
    void createCartAI_Unauthorized() throws Exception {
    ...
    void  getAllItemCart_Success() throws Exception {
        ...
        when(cartService.getAllItemCart(anyInt(), ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.OK).body(listItemCartResponse));
        mockMvc.perform(MockMvcRequestBuilders.get(endPointPath + "/list-cartItem/1?&language=VN"))
                .andExpect(status().isOk())
    ...
    void  getAllItemCart_NotFound() throws Exception {
        when(cartService.getAllItemCart(anyInt(), ArgumentMatchers.any(Language.class))).thenReturn((ResponseEntity) ResponseEntity.status(HttpStatus.NOT_FOUND).body("Not found cart"));
        mockMvc.perform(MockMvcRequestBuilders.get(endPointPath + "/list-cartItem/1?&language=VN"))
                .andExpect(status().isNotFound())
